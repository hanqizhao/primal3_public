...
def one_step(env, one_episode_perf, actions, pre_block, num_agent):
    """run one step"""
    train_valid = np.zeros((num_agent, EnvParameters.N_ACTIONS), dtype=np.float32)
    obs, vector, rewards, done, next_valid_actions,  blockings, _, num_blockings, leave_goals, \
        num_on_goal, max_on_goal, num_collide, action_status \
